...
Fourth release: modernized packaging with a single source of truth, platform build scripts, WiX (latest) on Windows, and multi-distro Linux support.
...
- **Platform build scripts:** Packaging logic moved out of the workflow into scripts you can run locally:
  - **Windows:** `packaging/windows/build-msi.ps1` (WiX, optional .ico generation from PNG).
...
- **Windows: WiX:** MSI is built with WiX (`dotnet tool install --global wix`; UI extension WixToolset.UI.wixext 6.0.2). Icon: `react.ico` is generated from `packaging/icons/react.png` when missing.
