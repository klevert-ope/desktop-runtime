...
- **`windows/`** – WiX 4 installer (MSI), License.rtf, and `build-msi.ps1`.
...
- **Windows:** WiX 4 (`dotnet tool install --global wix`). Optional: tool to generate `.ico` from PNG if not using the built-in step in `build-msi.ps1`.
